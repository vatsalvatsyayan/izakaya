import { BASE_INFERENCE_LATENCY, GPU_TDP } from './constants';
/**
 * PUE = totalFacilityPower / itEquipmentPower
 * Clamped to minimum 1.0 (physically impossible to be lower).
 */
export function computePUE(totalFacilityPower, itEquipmentPower) {
 ...
 */
export function computeWUE(waterUsageRate, itEquipmentPower) {
    if (itEquipmentPower <= 0)
        return 0;
 ...
 * Clamped to minimum 0.
 */
export function computeCUE(totalFacilityPower, itEquipmentPower, gridCarbonIntensity) {
    if (itEquipmentPower <= 0)
        return 0;
 ...
 * Unit: kgCO2.
 */
export function computeCarbonOutput(totalFacilityPower, gridCarbonIntensity, timeIntervalHours) {
 ...
 * Unit: kW.
 */
export function computeGpuIdlePowerWaste(gpuUtilization, activeGpuCount) {
 ...
 */
export function computeInferenceLatency(avgGpuTemp, queueDepth, batchSize) {
 ...
 * Unit: kW.
 */
export function computeCoolingPower(baseCoolingPower, ambientTemp, fanSpeedPercent, waterRecircMode) {
    return Math.max(0, baseCoolingPower *
 ...
 * Unit: L/hr.
 */
export function computeWaterUsageRate(coolingPower, coolingSetpoint, ambientTemp, waterRecircMode) {
 ...
 * Unit: kW.
 */
export function computeITEquipmentPower(activeGpuCount, gpuPowerLimit, gpuUtilization) {
 ...
 */
export function computeBatchEfficiency(batchSize, gpuUtilization) {
 ...
 * Unit: °C.
 */
export function computeGpuTemperature(coolingSetpoint, gpuPowerLimit, gpuUtilization, ambientTemp) {
 ...
 */
export function computeRequestDropRate(requestVolume, requestRateLimit, activeGpuCount, gpuUtilization) {
    if (requestVolume <= 0)
        return 0;
 ...
 */
export function computeQueueDepth(requestVolume, activeGpuCount, gpuUtilization, batchSize) {
 ...
 */
export function determineHealthStatus(value, thresholds) {
 ...
 */
export function computeCommunityBurden(locationState, waterUsageRate) {
    const facilityWaterDrawLitersPerDay = waterUsageRate * 24;
    let waterStressLevel;
    if (locationState.waterStressIndex < 0.3)
        waterStressLevel = 'low';
    else if (locationState.waterStressIndex < 0.6)
        waterStressLevel = 'moderate';
    else if (locationState.waterStressIndex < 0.8)
